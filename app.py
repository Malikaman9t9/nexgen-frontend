...
if menu_selection == "Site Auditor":
    ...
    if run_button and domain_input:
        ...
        with t1:
            ...
            with c1: st.markdown(render_audit_card_vip("Title Tag", get_audit_status_vip(onpage, "title")), unsafe_allow_html=True)
            with c2: st.markdown(render_audit_card_vip("Canonical URL", get_audit_status_vip(onpage.get('canonical','Missing'), "canonical")), unsafe_allow_html=True)
        with t3:
            if not is_pro:
